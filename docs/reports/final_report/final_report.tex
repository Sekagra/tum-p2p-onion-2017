...
\tikzstyle{orchestrator} = [rectangle, draw=black, text=white, fill=darkestBrew, rounded corners=15pt, text width=7.6em, minimum height=7.6em, minimum width=7.6em]
...
\resizebox{\textwidth}{!}{\begin{tikzpicture}[x=1mm,y=1mm, >=latex, ->]
    \node[orchestrator] (orchestrator) {\hspace{1.5mm}\large{Orchestrator} \\ \vspace{2mm} \hspace{0.3mm}\scriptsize{$\bullet$ handles rounds}\,\raisebox{-0.85ex}{\Large{\clock}} \\ \hspace{0.3mm}\scriptsize{$\bullet$ holds state}};
