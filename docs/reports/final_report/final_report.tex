...
If we are using at least two intermediate hops, to peer in the tunnel is able to know the address of the sender and receiver of data.
In addition, we do not transfer any information that allows the identification of the tunnel initiator.
As a result, the onion module provides sender anonymity.
...
If only one tunnel is established over a specified peer at a time, it is possible to track a tunnel using the \texttt{ONION INIT} and \texttt{ONION ACCEPT} messages.
However, this attack is no longer possible if multiple tunnels are established over the same hop at the same time.
Therefore, we have to rely on a large anonymity set, to prevent such timing- and pattern-based attacks.
...
As we also refresh cover tunnel at round transition, an attack is not able to differentiate of a peer is currently communicating or not.


\paragraph{Authenticity}
The VoidPhone application uses pre-shared public keys for the identification of peers.
In addition, the onion module transfers to handshake messages during tunnel establishment.
The onion authentication module can use those handshake messages and the pre-shared keys to provide mutual authentication of the communication partners.
...
\begin{table}[H]
    ...
    \begin{tabular}{l|l|l|p{5cm}}
        ...
        onion   & listen\_address       & <public\_ip>:<port>               & P2P address of this peer \\ \cline{2-4}
...
Execute the commands shown in Listing \ref{lst:source_build} in the \texttt{src/} directory, to build the project from source.
...
All unit tests and a large integration test that builds a tunnel, sends data and destroys it afterwards using mock-ups, are executed during Maven build.
...
Nevertheless, we have the opinion that both team spent an equal amount of time and the work was distributed fairly.
...
\paragraph{Marko}
    \begin{itemize}
        ...
        \item Integ: Marko developed mockups for all interfaces except the onion-to-onion interface to have an integration test for the internal onion logic.
    \end{itemize}
\paragraph{Together}
